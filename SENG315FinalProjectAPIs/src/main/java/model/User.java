	...
	public String updateUser() {
		...
		try {
			...
			String updateUser = "UPDATE "+mssqlConnection.getDatabase()+".dbo.users SET " +
				...
				"email=IsNull(NullIf(?,''),email), password=IsNull(NullIf(?,''),password), " +
				"username=IsNull(NullIf(?,''),username), active=IsNull(?,active) " +
				"WHERE userID="+userID+"";
			...
			while (lstResult.next()) {
				...
				if (dbPassword.equals(password)) {
					if (dbActive.equals("0")) {
						message = "This User is currently deactivated. Please contacat a System Administrator.";
					} else {
						message = "success";
					}
				}
